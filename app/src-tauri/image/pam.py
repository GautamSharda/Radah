...
SYSTEM_PROMPT_DEFAULT = f"""<SYSTEM_CAPABILITY>
...
* You are utilising a macbook using {platform.machine()} architecture with internet access.
* When viewing a page it can be helpful to zoom out so that you can see everything on the page.  Either that, or make sure you scroll down to see everything before deciding something isn't available.
* When using your computer function calls, they take a while to run and send back to you.  Where possible/feasible, try to chain multiple of these calls all into one function calls request.
* The current date is {datetime.today().strftime('%A, %B %-d, %Y')}.
</SYSTEM_CAPABILITY>
...
SYSTEM_PROMPT = SYSTEM_PROMPT_DEFAULT if not IS_MACOS else SYSTEM_PROMPT_MACOS
...
async def run_pam(message_queue = deque(), prompt: str = "", previous_messages: list[BetaMessageParam] = [], get_prompt_running = lambda: "running", MOCKDATA = False, system_prompt_suffix: str = ""):  # Need to make this async since sampling_loop is async
    ...
    def output_callback(content_block: BetaContentBlockParam) -> None:
        # Create a copy of the content block for printing
        print_block = content_block.copy()
        
        # If it's a tool result with an image, truncate or remove the base64 data
        if isinstance(print_block, dict):
            if print_block.get("type") == "tool_result" and "content" in print_block:
    ...
    def tool_output_callback(result: ToolResult, tool_id: str, previous_messages: list[BetaMessageParam] = []) -> None:
        ...
        print_result = ToolResult(
            ...
            error=result.error,
            base64_image="<base64_image_data_omitted>" if result.base64_image else None,
            system=result.system
        )
        print(f"Tool output: {print_result}, ID: {tool_id}")
